...
export default function PortfolioCaseStudy({
    ...
    openLightbox({
      ...
      panel: {
        ...
        relatedLinks: related.slice(0, 4).map((r) => ({
          label: r.title,
          sublabel: r.category,
          href: `/luxury-v4/portfolio/${r.slug}`,
        })),
      },
    });
  };
  ...
  return (
    <>
      {/* ── Hero ── */}
      <PageHero
        breadcrumbItems={[{ label: 'Home', href: '/luxury-v4' }, { label: 'Portfolio', href: '/luxury-v4/portfolio' }, { label: project.category }]}
      ...
      {related.length > 0 && (
        <GallerySection spacing="standard" background={luxoraColors.warmCream}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {related.map((r) => (
              <Link key={r.slug} href={`/luxury-v4/portfolio/${r.slug}`} className="group relative rounded-2xl overflow-hidden aspect-[4/5] block border" style={{ borderColor: 'rgba(160,120,80,0.16)' }}>
