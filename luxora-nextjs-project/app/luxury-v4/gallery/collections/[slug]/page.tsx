...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  return {
    ...
    alternates: { canonical: `/luxury-v4/gallery/collections/${collection.slug}` },
    ...
    <ServicePageShell>
      <GalleryBreadcrumbJsonLd
        items={[
          ...
          { label: collection.label, href: `/luxury-v4/gallery/collections/${collection.slug}` },
        ]}
      />

      <PageHero
        breadcrumbItems={[{ label: 'Home', href: '/luxury-v4' }, { label: 'Gallery', href: '/luxury-v4/gallery' }, { label: collection.label }]}
        badge="Collection"
      ...
      <GallerySection spacing="standard" background={luxoraColors.ivory} patterns={<LuxuryGrain id={`collection-${collection.slug}-grain`} opacity={0.012} />}>
        <GalleryGrid columns={{ desktop: 3 }}>
          {projects.map((project) => (
            <GalleryStandardCard
              key={project.id}
              href={`/luxury-v4/gallery/${project.category}/${project.slug}`}
