...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/luxury-v4/gallery' },
  openGraph: {
...
const styleMosaicItems: GalleryStyleMosaicItem[] = [...galleryStyles]
  ...
  .map((s) => ({
    ...
    href: `/luxury-v4/gallery?style=${encodeURIComponent(s.label)}#gallery-browser`,
  }));
...
export default function GalleryHomePage() {
  return (
    <ServicePageShell>
      ...
      <PageHero
        breadcrumbItems={[{ label: 'Home', href: '/luxury-v4' }, { label: 'Gallery' }]}
      ...
      <GallerySection id="browse-by-room" spacing="standard" background={luxoraColors.ivory}>
        ...
        <GalleryGrid columns={{ desktop: 3 }}>
          {roomCategories.map((cat) => (
            <GalleryStandardCard
              key={cat.slug}
              href={`/luxury-v4/gallery/${cat.slug}`}
      ...
      >
        ...
        <GalleryEditorialSplit
          featured={{
            href: `/luxury-v4/gallery/${editorsPickFeatured.category}/${editorsPickFeatured.slug}`,
          ...
          mini={editorsPickMinis.map((p) => ({
            href: `/luxury-v4/gallery/${p.category}/${p.slug}`,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {homeCollections.map((collection) => {
            ...
            return (
              <GalleryStandardCard
                key={collection.slug}
                href={`/luxury-v4/gallery/collections/${collection.slug}`}
